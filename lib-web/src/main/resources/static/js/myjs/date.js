...
//选择日期
function SelectDate(event,obj,strFormat,x,y) {
	leftX =(x == null) ? leftX : x; 
	topY =(y == null) ? topY : y; 
	if(document.getElementById("ContainerPanel")==null){InitContainerPanel();} 
	var date = new Date(); 
	var by = date.getFullYear()-70; //最小值
	var ey = date.getFullYear()+5; //最大值 
	...
	document.getElementById('calendarPanel').onclick = function(e) {
	    stopPropagation(e);
	}
} 
...
Calendar.prototype.draw = function() { 
	...
	var mvAry = [];
    mvAry[mvAry.length] = '<iframe id="iframe1" src="about:blank" frameBorder="0" marginHeight="0" marginWidth="0" style="position:absolute; visibility:inherit; top:0px;left:0px;width:175px; height:100%;z-index:-1; filter:alpha(opacity=0);"></iframe>';
	mvAry[mvAry.length] = ' <div name="calendarForm" style="margin: 0px;">';
	mvAry[mvAry.length] = ' <table height="20px" border="0" cellpadding="0" cellspacing="1" style="font-size:12px;">'; 
	mvAry[mvAry.length] = ' <tr width="100px;">'; 
	mvAry[mvAry.length] = ' <th align="left" width="1%" style="float: left"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;color:#000; line-height:16px;padding: 0'; 
	...
	mvAry[mvAry.length] = ' <th align="center" width="98%" nowrap="nowrap"><select name="calendarYear" onclick="stopPropagation()" id="calendarYear" style="font-size:12px;width: 65px; margin:0 6px 0 2px; padding: 0 2px; height: 20px;"></select><select name="calendarMonth" onclick="stopPropagation()" id="calendarMonth" style="font-size:12px;width: 65px;margin:0;padding: 0 2px; height: 20px;';
	if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏 “月” 
	mvAry[mvAry.length] = '"></select></th>'; 
	mvAry[mvAry.length] = ' <th align="right" width="1%" style="float: left"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;color:#000;line-height:16px;padding: 0'; 
	if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏 “月” 
	...
	mvAry[mvAry.length] = '<span style="float: left;' 
	if(calendar.DateMode >= pickMode["day"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到时日隐藏“时间” 
	mvAry[mvAry.length] = ' "><select name="calendarHour" id="calendarHour" style="font-size:12px;width: 38px; margin:0 5px 0 2px; padding: 0; height: 20px;"></select>' + Calendar.language["hour"][this.lang]+'</span>';  
	mvAry[mvAry.length] = '<span style=" float: left;' 
	if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时时隐藏 “分” 
	mvAry[mvAry.length] = '"><select name="calendarMinute" id="calendarMinute" style="font-size:12px;width: 38px; margin:0 5px 0 2px; padding: 0; height: 20px;"></select>' + Calendar.language["minute"][this.lang]+'</span>'; 
	mvAry[mvAry.length] = '<span style="float: left;' 
	if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时、分时隐藏 “秒” 
	mvAry[mvAry.length] = '"><select name="calendarSecond" id="calendarSecond" style="font-size:12px;width: 38px; margin:0 5px 0 2px; padding: 0; height: 20px;"></select>'+ Calendar.language["second"][this.lang]+'</span>'; 
	...
	mvAry[mvAry.length] = ' <input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer;color:#000; padding: 0 4px;"/>'; 
	...
	mvAry[mvAry.length] = '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer;color:#000;padding: 0 4px;"/>'; 
	mvAry[mvAry.length] = ' <input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer;color:#000; padding: 0 4px;"/>'; 
...
{
var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:12px;">';
str+='</div>';
if(document.all)
{ 
...
div.style.display ="none";
	var parentDiv = document.getElementById('withScroll');
	if(parentDiv != null && parentDiv != undefined && parentDiv != ''){
		document.getElementById('withScroll').appendChild(div);
	}else{
...
function dateFormat(timestempStr,formater){
	var date = new Date(timestempStr);
